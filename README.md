...
- Explicit eye-level, +35°, and −35° passes at twelve headings each. Final assembly remains locked until all 36 target photos have been captured. Twelve headings keep neighbouring photographs overlapping by at least 40% on every phone measured, from a narrow 3:4 crop near 53° to an ultrawide near 90°, where eight left the narrow cameras under 25% and too thin to align across a plain wall.
...
- Consistent stills during the sweep: exposure, white balance, and focus lock to their current values when the camera supports manual modes, an instant Laplacian sharpness check drops motion-blurred automatic captures so the same target retakes immediately, and each still records the phone's live orientation sample.
- Full-resolution photo capture through `ImageCapture.takePhoto()` when the browser exposes it, with the live-preview canvas grab as the universal fallback, plus an automatic under-exposed companion still per target on manual-exposure cameras so the laptop can Mertens-fuse blown-out window highlights back into the panorama.
- Laptop-side panorama assembly — 3072×1536, or 4096×2048 when the phone delivered full-resolution photo stills — with validated uploads, SIFT overlap alignment, IMU-guided placement priors with per-frame roll correction, cross-band registration that measures each tilted sweep's true yaw offset and pitch against the eye-level ring, spherical projection, exposure compensation, graph-cut seam selection, multiband blending, smooth polar caps, quality reporting, targeted retake requests, shared project persistence, and JPG download. A standard capture assembles in seconds. Successful projects retain their 36 source photographs (with recorded orientation samples and exposure brackets), manifest, report, and panorama under `.astra3d-data/projects/`.
